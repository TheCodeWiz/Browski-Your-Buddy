...
import SyntaxHighlighter from "react-syntax-highlighter"; // default highlighter, no prism
import { dark } from "react-syntax-highlighter/dist/cjs/styles/hljs"; // use hljs dark theme instead
...
const CodeComponent: React.FC<CodeProps> = ({ inline, className, children }) => {
  const match = /language-(\w+)/.exec(className || "");
  const language = match ? match[1] : "";

  if (inline) {
    return <code className="bg-[#2d2d2d] px-1 rounded text-sm">{children}</code>;
  }
  ...
  return (
    <div className="relative bg-[#23272a] rounded-lg p-4 my-2 shadow-md">
      ...
      <SyntaxHighlighter
        language={language}
        style={dark} // using hljs dark theme instead of prism
        PreTag="div"
        customStyle={{
          margin: 0,
...
const MarkdownMessage: React.FC<MarkdownMessageProps> = ({ content }) => {
  ...
  const components: Components = {
    ...
    blockquote: ({ children }) => (
      <blockquote className="border-l-4 border-gray-600 pl-4 my-2 italic">{children}</blockquote>
    ),
  };
